...
class TPSGridGen(nn.Module):
    def __init__(self, out_h=28, out_w=28, grid_size=3, device="cuda"):
        ...
        self.gridX = torch.FloatTensor(self.gridX).unsqueeze(0).unsqueeze(3).to(self.dv).double()
        self.gridY = torch.FloatTensor(self.gridY).unsqueeze(0).unsqueeze(3).to(self.dv).double()
        ...
        self.PX = PX.unsqueeze(2).unsqueeze(3).unsqueeze(4).transpose(0,4).to(self.dv).double() # 1 * 1 * 1 * 1 * N
        self.PY = PY.unsqueeze(2).unsqueeze(3).unsqueeze(4).transpose(0,4).to(self.dv).double()
    ...
    def compute_L_inverse(self, X, Y):
        ...
        Li = torch.inverse(L).to(self.dv).double()
        return Li
